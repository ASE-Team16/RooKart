...
public class task_search_result_service extends Activity {

    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_task_search_result_service);
        ...
        for(int i=0;i<j.size();i++)
            try {
                Log.d("",j.get(i).toString());
                jobs[i]="Title:"+j.get(i).getString("title")+"\nDescription:"+j.get(i).getString("description")+"\nMobile:"+j.get(i).getString("time")+" \nEmail:"+j.get(i).getString("mail")+" \nAmount:"+j.get(i).getString("price")+" $";
            } catch (JSONException e) {
                e.printStackTrace();
            }
        ...
        l.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                try{
                    JSONObject temp1=j.get((int) id);
                    String tid=(temp1.get("mail")).toString();
                    String tid1=(temp1.get("time")).toString();
                    String tid2=(temp1.get("title")).toString();

                    Intent i = new Intent(Intent.ACTION_SEND);
                    i.setType("message/rfc822");
                    i.putExtra(Intent.EXTRA_EMAIL  ,new String[]{tid});
                    i.putExtra(Intent.EXTRA_SUBJECT, "Want to use your service"+tid2 );
                    i.putExtra(Intent.EXTRA_TEXT   , "Please reach me back to "+tid1);

                    startActivity(Intent.createChooser(i, "Send mail..."));
                } catch (android.content.ActivityNotFoundException ex) {

                    Toast.makeText(task_search_result_service.this, "There are no email clients installed.", Toast.LENGTH_SHORT).show();
                } catch (JSONException e) {
                    e.printStackTrace();}
...
//                search_connection_service con= new search_connection_service();
//                try {
